...
public class TeamController {
    ...
    // 팀 생성
    @PostMapping
    public ResponseEntity<Void> createTeam(@Valid @RequestBody TeamCreateReqDto teamCreateReqDto,
    ...
    // 멤버가 속한 팀 목록 가져오기
    @GetMapping
    public RspTemplate<List<TeamMemberRspDto>> getTeams(@AuthenticationPrincipal Long memberId) {
        List<TeamMemberRspDto> teamRspDtos = teamService.findTeamRspDtosByMemberId(memberId);
        return new RspTemplate<>(HttpStatus.OK, "소속 팀들 조회", teamRspDtos);
    }

    // 팀 업데이트
    @PatchMapping
    public ResponseEntity<Void> updateTeam(@Valid @RequestBody TeamUpdateReqDto teamUpdateReqDto,
                                     @AuthenticationPrincipal Long memberId) {
    ...
    // 팀원들 스케쥴 조회
    @GetMapping("/schedule")
    public  ResponseEntity<List<List<TeamScheduleRspDto>>> getTeamSchedule(@Valid @RequestBody TeamScheduleListReqDto teamScheduleListReqDto,
                                           @AuthenticationPrincipal Long memberId) {
    ...
    public ResponseEntity<Void> deleteTeamMember(@PathVariable("teamId") Long teamId,
                                           @AuthenticationPrincipal Long memberId) {
    ...
    public ResponseEntity<Void> joinTeam(@PathVariable("teamCode") String receiveTeamCode,
                                   @AuthenticationPrincipal Long memberId) {
...
//    public ResponseEntity updateTeamSequence(@RequestBody) {
//
//    }
}
